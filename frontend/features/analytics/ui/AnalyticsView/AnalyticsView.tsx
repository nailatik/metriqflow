...
type ViewsByDay = { date: string; views: number; reactions: number; forwards: number; posts: number };
type TopPost    = { message_id: number; text: string | null; views: number; reactions_total: number; forwards: number; posted_at: string; has_media: boolean };
...
const NEW_CHANNEL_MS   = 30 * 60 * 1000; // 30 min — show banner if added recently
...
export function AnalyticsView() {
  ...
  // Fetch on channel/period change
  useEffect(() => {
    setAnalytics(null);
    fetchAnalytics();
  }, [fetchAnalytics]);

  // Auto-refresh every 10 min when period = 24h
  useEffect(() => {
